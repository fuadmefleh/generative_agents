...
class ReverieServer: 
  ...
  def start_server(self, int_counter): 
    ...
    while (True): 
      ...
      # Create the movement folder if it doesn't exist.
  ...
  def open_server(self): 
    ...
    while True: 
      sim_command = "run 500" #input("Enter option: ")
...
if __name__ == '__main__':
  ...
  # Remove prevous target simulation folder if it exists.
  target = "abcd"
  while os.path.exists(f"{fs_storage}/{target}"):
    # Delete the folder
    shutil.rmtree(f"{fs_storage}/{target}")
    print (f"Deleted existing simulation folder: {target}")
